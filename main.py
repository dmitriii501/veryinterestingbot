...
async def main():
    ...
    try:
        if not app_settings.SUPABASE_URL or not app_settings.SUPABASE_KEY:
            logging.error(
                "SUPABASE_URL и/или SUPABASE_KEY не установлены в конфигурации."
            )
    ...
    if hasattr(start, "router"):
        ...
        logging.error(
            "Роутер из bot.handlers.start не найден или не имеет атрибута 'router'."
        )
    ...
    if hasattr(help_command, "router"):
        ...
        logging.error(
            "Роутер из bot.handlers.help не найден или не имеет атрибута 'router'."
        )

    if hasattr(ai_query_handler, "router"):
        dp.include_router(ai_query_handler.router)
        logging.info(
            f"Роутер '{ai_query_handler.router.name or 'ai_query_handler'}' зарегистрирован."
        )
    else:
        logging.error(
            "Роутер из bot.handlers.ai_query не найден или не имеет атрибута 'router'."
        )

    if hasattr(employees_handler, "router"):  # <--- ИЗМЕНИТЕ НА ИМЯ ВАШЕГО ФАЙЛА
        dp.include_router(employees_handler.router)
        logging.info(
            f"Роутер '{employees_handler.router.name or 'employees_handler'}' зарегистрирован."
        )
    else:
        logging.error(
            "Роутер из bot.handlers.employees_handler не найден или не имеет атрибута 'router'."
        )
    ...
    if hasattr(nlu_handler, "router"):
        ...
        logging.info(
            f"Роутер '{nlu_handler.router.name or 'nlu_handler'}' зарегистрирован."
        )
    else:
        logging.error(
            "Роутер из bot.handlers.nlu_handler не найден или не имеет атрибута 'router'."
        )  # ОШИБКА
